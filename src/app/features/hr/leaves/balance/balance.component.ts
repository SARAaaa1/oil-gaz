  ...
  template: `
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-black text-slate-800 tracking-tight">{{ 'hr.leave_balance.title' | translate }}</h1>
          <p class="text-xs text-slate-500 font-semibold mt-1">{{ 'hr.leave_balance.subtitle' | translate }}</p>
        </div>
      </div>
...
export class HrLeaveBalanceComponent implements OnInit {
  private readonly breadcrumb = inject(BreadcrumbService);

  ngOnInit() {
    this.breadcrumb.setBreadcrumbs([
      { label: 'navigation.hr' },
      { label: 'hr.leave_balance.title' }
    ]);
  }
}
