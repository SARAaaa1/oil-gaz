...
@Component({
  selector: 'app-hr-employee-evaluations',
  standalone: true,
  ...
  template: `
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-black text-slate-800 tracking-tight">{{ 'hr.employee_evaluations.title' | translate }}</h1>
          <p class="text-xs text-slate-500 font-semibold mt-1">{{ 'hr.employee_evaluations.subtitle' | translate }}</p>
        </div>
      </div>
...
export class HrPerformanceEvaluationsComponent implements OnInit {
  private readonly breadcrumb = inject(BreadcrumbService);

  ngOnInit() {
    this.breadcrumb.setBreadcrumbs([
      { label: 'navigation.hr' },
      { label: 'hr.employee_evaluations.title' }
    ]);
  }
}
