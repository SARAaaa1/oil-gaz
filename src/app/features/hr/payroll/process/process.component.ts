...
@Component({
  selector: 'app-hr-payroll-processing',
  standalone: true,
  ...
  template: `
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-black text-slate-800 tracking-tight">{{ 'hr.payroll_processing.title' | translate }}</h1>
          <p class="text-xs text-slate-500 font-semibold mt-1">{{ 'hr.payroll_processing.subtitle' | translate }}</p>
        </div>
      </div>
...
export class HrPayrollProcessComponent implements OnInit {
  private readonly breadcrumb = inject(BreadcrumbService);

  ngOnInit() {
    this.breadcrumb.setBreadcrumbs([
      { label: 'navigation.hr' },
      { label: 'hr.payroll_processing.title' }
    ]);
  }
}
